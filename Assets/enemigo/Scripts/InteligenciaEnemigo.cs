...
public class InteligenciaEnemigo : MonoBehaviour {
	...
	public float x, y, z;
	Vector3 offsetBuscador=new Vector3(0,1.5f,0);
				...
				foreach (GameObject enGo in Enemies) {
					...
					if (angle < 110f * 0.5f ) {//angulo al que debe estar el enmigo
						...
						if (Physics.Raycast (transform.position + transform.up, direction.normalized, out hit, 20)) {//20 es el radio de la mira
							...
							if (hit.collider.GetComponent<StatsDeLosPersonajes> ()) {//no funciona esta linea
								Debug.Log(" enemigo");
								if (hit.collider.GetComponent<StatsDeLosPersonajes> ().Id != GetComponent<StatsDeLosPersonajes> ().Id) {
									Debug.Log ("!!!!!!!!!!!!!!!");
	...
	void OnAnimatorIK(){
		if (enemyToAtack) {
			anim.SetLookAtWeight (.2f, .2f, 1, 1, 1);
			//Debug.Log("enemigo a atatacar");
			anim.SetLookAtPosition (lastKnowPosition-new Vector3(x,0,z));
	...
	void shootRay(){
		...
		Debug.DrawRay (startPos, directionToAttack,  Color.cyan,0.2f);
		if(Physics.Raycast(startPos,directionToAttack,out hit,Mathf.Infinity,mask)){
			...
			if (hit2.transform.root.GetComponent<StatsDeLosPersonajes> ()) {
				StatsDeLosPersonajes sta = hit2.transform.root.GetComponent<StatsDeLosPersonajes> ();
				sta.health -= 50;
			}
		}
		EndPos=hit.point;
...
void LookForEnemy(){
	...
	RaycastHit hit;
		Vector3 directionNueva = enemyToAtack.transform.position - offsetBuscador;//para que apunte mas abajo 
	...
	if (Physics.Raycast (weponManger.activeWeapon.bulletSpawn.TransformPoint(Vector3.zero), direction, out hit)) {
		if (hit.transform.root.GetComponent<StatsDeLosPersonajes> ()) {
			if (hit.transform.root.GetComponent<StatsDeLosPersonajes> ().Id != GetComponent<StatsDeLosPersonajes> ().Id) {
				Debug.Log ("en vision");
				lastKnowPosition = enemyToAtack.transform.position;
					lastKnowPosition.y = 0;
				InSIght = true;
			} else {
